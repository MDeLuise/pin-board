...
public class ApiKeyService {
    ...
    public void remove(User user, Long id) {
        ApiKey toRemove = repository.findById(id).orElseThrow(() -> new EntityNotFoundException(id));
    ...
    public ApiKey get(User user, Long id) {
        ApiKey toReturn = repository.findById(id).orElseThrow(() -> new EntityNotFoundException(id));
    ...
    public User getUserFromApiKey(String apiKeyValue) {
        ApiKey apiKey =
            repository.findByValue(apiKeyValue).orElseThrow(() -> new EntityNotFoundException("value", apiKeyValue));
